"""Integration tests for the /runs skeleton endpoint (backward compat)."""
import pytest
from sqlalchemy.orm import Session

from graph.runner import run_agent_text
...
def test_pipeline_runs_end_to_end(_isolated_db):
    run_id = run_agent_text("Explain why the sky is blue in one sentence.")
...
def test_pipeline_stores_input(_isolated_db):
    ...
    run_id = run_agent_text(input_text)
    with Session(session_module._engine) as s:
        run = s.get(RunRow, run_id)
