...
- **Gate (all must pass):**
  ...
  8. **If the agent accesses external data (CSV, DB, API, files): the loop is a ReAct loop, not a single-shot call.** The stub must simulate at least two iterations: one where the LLM generates an action, and one where it emits `FINAL ANSWER:`. A stub that returns a final answer on the first call without executing any action does not validate the loop — it hides a design defect that will surface as wrong answers in Phase 3. See `spec/engineering/ai-agents.md` Section 10 for the full pattern.
