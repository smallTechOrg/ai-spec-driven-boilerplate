"""ReAct nodes — plan_action -> invoke_tool ↺ -> finalize / handle_error.

The LLM emits a JSON plan ({"tool", "parameters", "reasoning"}) or signals
completion ({"tool": "FINAL_ANSWER", "result"}). Tools fail soft. This is the
storage-layer-agnostic agent code (identical across both Python recipes).
"""
...
async def invoke_tool(state: AgentState) -> dict:
    ...
    async with span(f"execute_tool.{tool_name}", "TOOL", args=params) as sp:
        if not tool:
        ...
        sp["result_preview"] = str(tool_result)[:300]

    return {"tool_call_history": history + [{"role": "tool", "content": str(tool_result)}]}


async def finalize(state: AgentState) -> dict:
    log.info("run_complete", run_id=state["run_id"], result_len=len(state.get("result") or ""))
    return {}
