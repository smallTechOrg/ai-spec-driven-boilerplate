...
- **Tailwind v4 requires `postcss.config.mjs` (plugin: `@tailwindcss/postcss`) and `@source "../";` in the global CSS file.** Without both, the built CSS has no utility classes — the UI renders unstyled even though the build exits 0. The gate must verify the built CSS contains real utility selectors, not just check HTTP 200.
