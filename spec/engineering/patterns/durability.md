...
- **Default:** `SqliteSaver` (zero-ops, matches the SQLite default stack). Upgrade to `PostgresSaver`
  when the DB is PostgreSQL. Pick from [`../tech-stack.md`](../tech-stack.md) § Agentic Stack Tech.
...
- **Idempotency** — external side-effecting actions (sends, payments) carry an **idempotency key**
  (derived from `run_id` + action) so a resume-after-crash doesn't double-fire. This is the safety net
  that makes resumption safe.
