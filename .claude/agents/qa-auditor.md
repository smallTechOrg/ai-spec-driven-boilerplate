...
3. **Real-key check** (from the real-provider gate onward — any phase whose core loop calls a live provider) — the gate runs against the REAL LLM/API using keys from `.env`, and against the **production DB driver** (not SQLite if prod is PostgreSQL). A required key missing from `.env` → BLOCKED with the exact key name. Never substitute SQLite for a production DB.
4. **Golden-path + live-server + UI smoke** (at the real-provider gate and any later phase with a UI/HTTP surface) — run the primary user journey against the real LLM/API via `TestClient` asserting **response content** not just status; exercise edge cases and at least one full end-to-end path; for any UI surface assert rendered content and empty/loading/error states; then start the app and `curl` `/health` + one real page (both 200). **Derive the smoke from the phase's CAPABILITIES, not just its endpoints — test the capability, not one call to it.** If any capability the phase claims is **stateful** (persistent sessions, history, memory, conversation, accumulation, "across restart"), a single happy-path call does NOT gate it: you MUST exercise a **multi-interaction sequence** (≥2 operations in the SAME session — e.g. ask, then ask again; upload, then query, then query again) AND a **state-survival check** (reload the page and/or restart the process, then confirm prior state is still there). State-dependent bugs (detached ORM rows, stale caches, history-load crashes, session-scoping errors) are invisible to the first call and only surface on the second — a one-shot smoke that passes is not evidence the capability works. **For a static-export UI served by the backend, a 200 + HTML structure is NOT sufficient — it does not detect missing CSS/JS.** Gate the EXACT single-origin path the user runs (`pnpm build` → serve via the backend → `http://localhost:8001/app/`, not the `pnpm dev :3000` flow), and verify the page is actually **rendered AND styled**: assert the built CSS bundle contains real utility classes (e.g. `.flex`, `.bg-*`, `.rounded-*`) and **no unexpanded `@tailwind` directive**, and/or capture a headless-Chrome screenshot and confirm it isn't barebones. An unstyled page that returns 200 is a BLOCKER, not a pass.
...
6. **Cross-cutting Definition of Done** (every slice, per `harness/patterns/phases.md` Horizontal Axis):
   - (a) **README** — `README.md` is updated for any new command/setup/env/route/capability this phase added, AND every README command touched is re-run from the stated directory. A stale README is a **BLOCKER**.
   - (b) **Observability** — scan the slice diff for new call-sites (LLM/DB/HTTP/agent-node/endpoint/CLI); each must emit a structured log line via the observability module. A new operation with no log line is a **BLOCKER**. (Proportional: Phase 1 logs only its one core operation.)
   - (c) **Error handling** — each new external call has try/except + timeout and degrades gracefully.
   - (d) **Test** — the slice ships a real behaviour-asserting test (response content / DB state), not just status codes.
7. **Spot-check** — working tree state sane, no secrets in code, files match the plan for this scope, no later-phase code in this phase, and `.env` is gitignored (no real keys committed).

**Output:** `Scope: <slice / whole phase>`; `Code review` → CLEAN / BLOCKERS (file:line + concrete fix); `Gate: <cmd>` → PASS/FAIL (with real output tail); Smoke (real-key) → PASS/FAIL/N/A; First-time-right (exact user path) → PASS/FAIL; DoD (README+observability) → PASS/FAIL; **Verdict: VERIFIED / BLOCKED**. VERIFIED only with zero review blockers, a green gate, AND the exact user-tested path working first time with zero rough edges. A missing required key → BLOCKED naming the key. If BLOCKED, list exact findings/failures (file:line, test names, assertions, missing files, missing keys) so the responsible generator fixes without re-discovery.
...
- **Passing a UI on a 200 + HTML check alone — never confirming it actually renders STYLED** (utilities present in the built CSS / non-barebones screenshot) on the single-origin path the user runs. A clean build and a 200 do not prove the CSS/JS loaded.
...
- Passing a slice that added a command / route / capability with a **stale README**, or added an operation (LLM/DB/HTTP/node/endpoint/CLI) with **no structured log line** — the Horizontal-axis Definition of Done is not satisfied.
