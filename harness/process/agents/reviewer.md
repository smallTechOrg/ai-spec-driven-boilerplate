...
- **Reviews the spec before any code exists** (pre-code gate, below) — the cheapest place
  to catch a defect
- Reviews `src/` against `spec/` — the full gate runs **once, at the phase boundary**, when
  the steps converge into a user-testable whole (not a heavy per-step tax)
- **Loads the real UI in the browser path, not just the build.** If the phase has a frontend, the
...
- Confirms the **README is current** at the phase gate — every command works as written
...
- **Per phase (this gate):** the full fixed checklist + evals + README + live-server smoke,
...
- The phase gate is signed off in the session report
- Deployer can proceed

## Authority & boundaries

- **Tools:** Read, Bash (run tests), Write (acceptance tests, sign-off in the session report).
- **May write:** acceptance tests, the gate sign-off, the Reviewer-sign-off cells of
  `logs/PLAN.md` `## Progress Tracker`, and the `## Phase Acceptance` section of `logs/PLAN.md`.
...
Before the planner slices the phase, review the phased spec files (vision.md, architecture.md,
data-model.md, api.md, ui.md, agent-graph.md, and the `PN-ACn` criteria in
`spec/delivery-plan.md`) for the four requirement-bug classes — a vague spec causes large,
documented correctness drops in generated code, and a defect caught here costs minutes instead
of phases:
...
4. **Incompleteness** — a named capability with no `PN-ACn` criterion in `spec/delivery-plan.md`,
...
A phase that touches agent behaviour must pass the project's `evals/` golden cases at the
