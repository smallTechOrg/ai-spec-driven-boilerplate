...
def _offline_kill_switch():
    ...
    real async engine on the real driver (honours gotcha C-DB-SAME-AS-PROD).
    """
    os.environ.setdefault("APPNAME_LLM_PROVIDER", "stub")
...
async def db_session(monkeypatch, tmp_path) -> AsyncSession:
    ...
    ``AsyncSessionLocal`` so request handlers use the same store.
    """
