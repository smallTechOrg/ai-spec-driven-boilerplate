...
| [planner](agents/planner.md) | Slices the phase into a parallel step DAG |
| [executor](agents/executor.md) | Implements one step in `src/` (many run in parallel) |
| [reviewer](agents/reviewer.md) | Guards verification, signs off the phase gate |
...
A build runs as a **staged pipeline inside a phase loop**. The supervisor runs
