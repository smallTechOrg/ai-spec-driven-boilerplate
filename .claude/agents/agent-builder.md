...
3b. **Adversarially verify every correctness blocker before acting on it.** A single auditor both misses real bugs and raises false ones, so for each blocker finding a qa-auditor returns that touches **correctness** (skip style/nit findings), fan out **2–3 independent qa-auditor skeptics IN ONE MESSAGE** (parallel — free on wall-clock), each scoped to that one finding and prompted to **REFUTE it** ("here is a claimed bug at file:line; try to prove it is NOT a real defect — default to refuted if uncertain"). Keep the finding only if a **majority confirm it is real**; drop the ones the skeptics refute. Route only confirmed findings to the responsible generator's fix loop. This runs only on blocker findings (the exception, not every slice), so it adds parallel verification without serializing the build. You — agent-builder — own this fan-out; qa-auditor stays read-only and never spawns agents.
...
- Looping a generator on a single auditor's unverified correctness/security finding without the adversarial refutation pass (false positives waste fix cycles; lone-reviewer misses ship bugs).
